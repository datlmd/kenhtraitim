...
function dang_ky() {
  ...
  var txtparent = $.trim($('#parent_id').val());
...
// $dbname = "flicportal_db";
  ...
  {
      $sql_edit = "SELECT * FROM user_courses WHERE id=" . $_GET['id'];
    ...
    <div class="container">
          ...
          <h3 class="sub-header">Danh sách khoá học </h3>
                      ...
                      <tr style="background-color:#fff; border-top: 0px solid #fff;">
                        ...
                        <td style="border-top: 0px solid #fff;">
                          <?php
                            ...
                            if (isset($_GET['id'])  && $_GET['id'] > 0)
                              echo '<input id="name" name="name" class="form-control" type="text" placeholder="Khoá học" value="' . $row_edits[0]['name'] . '" />';
                            else
                              echo '<input id="name" name="name" class="form-control" type="text" placeholder="Khoá học" value="" />';
                          ?>
                        </td>
                        <td style="border-top: 0px solid #fff;">
                          <select id="parent_id" name="parent_id" class="form-control">
                            <option value="-1">-----Chọn khoá học cha-----</option>
                          <?php
                            ...
                            if ($result->num_rows > 0) {
                              ...
                              foreach($rows as $item) { 
                                if($_GET['id'] && $_GET['id'] > 0)
                                  if($row_edits[0]['parent_id'] == $item['id'])
                        ...
                        <td style="border-top: 0px solid #fff;">
                          <?php
                            if(isset($_GET['id'])  && $_GET['id'] > 0){
                              ...
                              echo '<a href="list_course.php" class="btn btn-success">Tạo mới</a>';
                            }
                            else
            ...
            <table class="table table-striped">
              ...
              <tbody>
              	<?php
              		...
              		$sql = "SELECT * FROM user_courses Order By id desc";
                  //var_dump($sql);
					$result = $conn->query($sql);
					if ($result->num_rows > 0) {
              $rows = null;
					  ...
					  foreach($rows as $item) { 
					  	echo '<tr>';
              echo '<td><a href="list_course.php?id=' . $item['id'] . '">Edit</a></td>';
					  	echo '<td>' . $item['id'] . '</td>';
