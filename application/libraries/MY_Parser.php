...
class MY_Parser extends CI_Loader {
    ...
    */
    ...
    {
        // PENGUIN - @hungtd
        // make to path template
        $template = theme_web() . '/' . CI::$APP->router->fetch_module() . '/' . $template;
    ...
    * @param mixed $caching
    */
    public function parse_layout($template, $data = array(), $return = TRUE, $caching = TRUE)
    {
        // PENGUIN - @hungtd
        // make to path template
        $layout_view_dir        = $this->CI->layout->layout_dir;
        $smarty_layout_dir      = substr($layout_view_dir, 6, strlen($layout_view_dir));
        $template = $smarty_layout_dir . $template;
