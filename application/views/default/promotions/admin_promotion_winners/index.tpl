...
<div class="heading">
    <h1>{lang('Admin promotion winners manager')}</h1>

    <div class=buttons>
        <a href="{base_url('promotions/admin_promotion_winners/add')}" class="button"><span>{lang('Add')}</span></a>
        ...
        {if $cf_names neq false}
            <select class="JsCustomFieldChange">
                {foreach $cf_names as $cfn}
                    <option value="{base_url('promotions/admin_promotion_winners/index/')}/{$cfn.id}" {if $cfn_id eq $cfn.id}selected{/if}>{$cfn.name}</option>
                {/foreach}
            </select>
        {/if}
    </div>
...
<div class="content">
            ...
            <tbody>
                <tr>                    
                    ...
                    <td>
                        ...
                        <input type="text" name="full_name" value="{$smarty.get.full_name}" />
                    </td>
    ...
    <form action="{base_url('promotions/admin_promotion_winners/delete')}" class="JsDeleteForm" method="post">
        {include file="./_list_view.tpl"}
    </form>
