...
<div class="heading">
    <h1>{lang('Admin promotion gift publics manager')}</h1>

    <div class=buttons>    	
        <a href="{base_url('promotions/admin_promotion_gift_publics/add')}" class="button"><span>{lang('Add')}</span></a>
        ...
        {if $cf_names neq false}
            <select class="JsCustomFieldChange">
                {foreach $cf_names as $cfn}
                    <option value="{base_url('promotions/admin_promotion_gift_publics/index/')}/{$cfn.id}" {if $cfn_id eq $cfn.id}selected{/if}>{$cfn.name}</option>
                {/foreach}
            </select>
        {/if}
    </div>
...
<div class="content">
    ...
    <form action="{base_url('promotions/admin_promotion_gift_publics/delete')}" class="JsDeleteForm" method="post">
