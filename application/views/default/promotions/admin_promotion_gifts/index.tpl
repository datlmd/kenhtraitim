...
<div class="heading">
    <h1>{lang('Admin promotion gifts manager')}</h1>

    <div class=buttons>
    	<a href="{base_url('promotions/admin_promotion_gift_publics')}" class="button"><span>{lang('Publish quà ra website')}</span></a>
        <a href="{base_url('promotions/admin_promotion_gifts/add')}" class="button"><span>{lang('Add')}</span></a>
        ...
        {if $cf_names neq false}
            <select class="JsCustomFieldChange">
                {foreach $cf_names as $cfn}
                    <option value="{base_url('promotions/admin_promotion_gifts/index/')}/{$cfn.id}" {if $cfn_id eq $cfn.id}selected{/if}>{$cfn.name}</option>
                {/foreach}
            </select>
        {/if}
    </div>
...
<div class="content">
    ...
    <form action="{base_url('promotions/admin_promotion_gifts/delete')}" class="JsDeleteForm" method="post">
