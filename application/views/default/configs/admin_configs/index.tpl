...
<div class="heading">
    <h1>{lang('Admin configs manager')}</h1>

    <div class=buttons>
        <a href="{base_url('configs/admin_configs/add')}" class="button"><span>{lang('Add')}</span></a>
        ...
        {if $cf_names neq false}
            <select class="JsCustomFieldChange">
                {foreach $cf_names as $cfn}
                    <option value="{base_url('configs/admin_configs/index/')}/{$cfn.id}" {if $cfn_id eq $cfn.id}selected{/if}>{$cfn.name}</option>
                {/foreach}
            </select>
        {/if}
    </div>
...
<div class="content">
    ...
    <form action="{base_url('configs/admin_configs/delete')}" class="JsDeleteForm" method="post">
