...
<div class="heading">
    <h1>{lang('Admin user logs manager')}</h1>

    <div class=buttons>
        <a href="{base_url('users/admin_user_logs/add')}" class="button"><span>{lang('Add')}</span></a>
        ...
        {if $cf_names neq false}
            <select class="JsCustomFieldChange">
                {foreach $cf_names as $cfn}
                    <option value="{base_url('users/admin_user_logs/index/')}/{$cfn.id}" {if $cfn_id eq $cfn.id}selected{/if}>{$cfn.name}</option>
                {/foreach}
            </select>
        {/if}
    </div>
...
<div class="content">
    ...
    <form action="{base_url('users/admin_user_logs/delete')}" class="JsDeleteForm" method="post">
