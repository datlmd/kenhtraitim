...
<div class="heading">
    <h1>{lang('Admin game logs manager')}</h1>

    <div class=buttons>
        <a href="{base_url('games/admin_game_logs/add')}" class="button"><span>{lang('Add')}</span></a>
        ...
        {if $cf_names neq false}
            <select class="JsCustomFieldChange">
                {foreach $cf_names as $cfn}
                    <option value="{base_url('games/admin_game_logs/index/')}/{$cfn.id}" {if $cfn_id eq $cfn.id}selected{/if}>{$cfn.name}</option>
                {/foreach}
            </select>
        {/if}
    </div>
...
<div class="content">
    ...
    <form action="{base_url('games/admin_game_logs/delete')}" class="JsDeleteForm" method="post">
