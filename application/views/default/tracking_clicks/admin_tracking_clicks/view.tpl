{*

/**
 * PENGUIN FrameWork
 * @author hungtd <tdhungit@gmail.com> 0972014011
 * @copyright Tran Dinh Hung 2011
 * 
 * View Tracking_clicks
 * 
 * @package PenguinFW
 * @subpackage tracking_clicks
 * @version 1.0.0
 */

*}

<div class="heading">
    <h1>{lang('View Admin tracking clicks')}</h1>

    <div class=buttons>
        <a href="{base_url()}users/admin_users/edit/{$data_view->id}" class="button"><span>{lang('Edit')}</span></a>
    </div>
</div>

<div class="content">
    <table class="list">
        <tbody>
            
            <tr>
                <td class="left head">{get_label('click_name')}</td>
                <td class="left">{pg_field_value($data_view->click_name, 'TEXT')}</td>
            </tr>

            <tr>
                <td class="left head">{get_label('created')}</td>
                <td class="left">{pg_field_value($data_view->created, 'DATETIME')}</td>
            </tr>

            <tr>
                <td class="left head">{get_label('modified')}</td>
                <td class="left">{pg_field_value($data_view->modified, 'DATETIME')}</td>
            </tr>

            <tr>
                <td class="left head">{get_label('ip')}</td>
                <td class="left">{pg_field_value($data_view->ip, 'TEXT')}</td>
            </tr>

            <tr>
                <td class="left head">{get_label('user_agent')}</td>
                <td class="left">{pg_field_value($data_view->user_agent, 'TEXT')}</td>
            </tr>

        </tbody>
    </table>
</div>
