...
<div class="heading">
    <h1>{lang('Music authors manager')}</h1>

    <div class=buttons>
        <a href="{base_url('musics/admin_music_authors/add')}" class="button"><span>{lang('Add')}</span></a>
        ...
        {if $cf_names neq false}
            <select class="JsCustomFieldChange">
                {foreach $cf_names as $cfn}
                    <option value="{base_url('musics/admin_music_authors/index/')}/{$cfn.id}" {if $cfn_id eq $cfn.id}selected{/if}>{$cfn.name}</option>
                {/foreach}
            </select>
        {/if}
    </div>
...
<div class="content">
    ...
    <form action="{base_url('musics/admin_music_authors/delete')}" class="JsDeleteForm" method="post">
