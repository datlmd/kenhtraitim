...
<div class="heading">
    ...
    <div class=buttons>
        <a href="{base_url('musics/admin_music_authors/add/1')}?insert_id={$smarty.get.insert_id}&insert_name={$smarty.get.insert_name}" class="button"><span>{lang('Add')}</span></a>        
    </div>
</div>
    ...
    <table class="list">
        ...
        <tbody>
            {foreach $list_views as $list_view}
                <tr>
                    <td class="left">
                        <a href="#this" onClick="getPopupData('JsPopup_author_id', '{$list_view.id}', 'JsPopup_author_name', '{$list_view.name}')">
                            {$list_view.name}
                        </a>
                    </td>
                </tr>
