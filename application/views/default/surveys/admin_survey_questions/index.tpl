...
<div class="heading">
    <h1>{lang('Survey Question manager')}</h1>
    
    <div class=buttons>                
        <a href="{base_url('surveys/admin_survey_questions/add')}" class="button"><span>{lang('Add')}</span></a>
...
<div class="content">
                    ...
                    <td>
                        ...
                        <select name="question_status_id">
                            ...
                            {foreach $question_status_ids as $question_status_id}
                                <option value="{$question_status_id.id}" {if $smarty.get.question_status_id != '' && $smarty.get.question_status_id == $question_status_id.id}selected{/if}>{lang($question_status_id.name)}</option>
                            {/foreach}                            
                        </select>
                    </td>
    ...
    <form action="{base_url('surveys/admin_survey_questions/delete')}" class="JsDeleteForm" method="post">
